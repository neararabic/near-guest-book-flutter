...
class LocalStorage{
  static saveKeys(KeyPair keyPair) async {
      ...
      List<String> privateKeyStrList = keyPair.privateKey.bytes.map((i) => i.toString()).toList();
      List<String> publicKeyStrList = keyPair.publicKey.bytes.map((i) => i.toString()).toList();

      await pref.setStringList(
          Constants.PRIVATE_KEY_STRING,
          privateKeyStrList);

      await pref.setStringList(
          Constants.PUBLIC_KEY_STRING,
          publicKeyStrList);
    }
  ...
  static Future<KeyPair?> loadKeys() async{
    ...
    List<int> privateKeyByteList = privateKeyStr.map((i) => int.parse(i)).toList();
    ...
    List<int> publicKeyByteList = publicKeyStr.map((i) => int.parse(i)).toList();
